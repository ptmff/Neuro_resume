<template>
  ...
  >
    <!-- Аватар -->
    <div class="relative w-28 h-28 md:w-32 md:h-32 rounded-full overflow-hidden border-2 border-[var(--profile-avatar-border)] shadow-md">
      <img
        :src="profile?.photo || '/placeholder.jpg'"
        alt="Avatar"
        class="w-full h-full object-cover"
      />
      <div class="absolute inset-0 rounded-full bg-[var(--background-cta)] opacity-20 blur-2xl animate-pulse"></div>
    </div>
    ...
    <div class="text-center md:text-left w-full max-w-xl">
      ...
      <div class="space-y-4 mb-4">
        <div v-for="key in editableKeys" :key="key">
          <label class="text-[var(--text-mainless)] text-sm">{{ labels[key] }}</label>
          <div v-if="isEditing">
            <input
          ...
          <p v-else class="text-[var(--text-light)] text-base">{{ profile?.[key] || '-' }}</p>
        </div>
      </div>
